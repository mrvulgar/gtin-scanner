#!/usr/bin/env python3
"""
Быстрое веб-приложение для извлечения Data Matrix кодов из PDF файлов
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gtin_scanner_fast.log'),
...
class GTINScanner:
    def __init__(self):
        ...
        logger.info("GTINScanner Fast инициализирован")
    ...
    def scan_pdf_fast(self):
        """Быстрое сканирование PDF"""
        logger.info("scan_pdf_fast начато")
        ...
        logger.info(f"Начинаем быстрое сканирование с областью: {self.crop_rect}")
        ...
        try:
            ...
            for page_num in range(total_pages):
                ...
                # Data Matrix коды обычно небольшие, поэтому не нужны огромные разрешения
            ...
            if all_codes:
                try:
                    ...
                    final_status = f"✅ Сканирование завершено за {total_time:.1f}с!\n📄 Страниц обработано: {total_pages}\n✅ Найдено кодов: {len(all_codes)}\n💾 Файл готов к скачиванию"
                    final_stats = f"Завершено: {total_pages}/{total_pages} | Кодов: {len(all_codes)} | Время: {total_time:.1f}с"
                    
                    logger.info("Возвращаем успешный результат")
                    return final_status, csv_content, final_stats
                    
                except Exception as csv_error:
                    logger.error(f"Ошибка при создании CSV: {csv_error}", exc_info=True)
                    error_status = f"❌ Ошибка при создании CSV: {str(csv_error)}\n📄 Страниц обработано: {total_pages}\n✅ Найдено кодов: {len(all_codes)}"
                    return error_status, None, "Ошибка при сохранении результатов"
            else:
                logger.warning("Коды не найдены")
                return "⚠️ Коды не найдены в выделенной области", None, "Проверьте выделенную область"
            
        except Exception as e:
            logger.error(f"Критическая ошибка при сканировании: {e}", exc_info=True)
            return f"❌ Ошибка при сканировании: {str(e)}", None, "Ошибка"
    ...
    def _generate_csv(self, codes):
        ...
        for i, code in enumerate(codes):
            ...
            logger.debug(f"Записан код {i+1}: {code}")
        
        csv_content = output.getvalue()
        output.close()
        
        logger.info(f"CSV сгенерирован: {len(csv_content)} байт")
        return csv_content
...
scanner = GTINScanner()
logger.info("GTIN Scanner Fast приложение запущено")


# Создаем интерфейс Gradio
with gr.Blocks(title="GTIN Scanner Fast", theme=gr.themes.Soft()) as app:
    gr.Markdown("""
    # ⚡ GTIN Scanner Fast - Быстрое извлечение Data Matrix кодов из PDF
    
    ### Инструкция:
    1. **Загрузите PDF файл** в поле ниже (рекомендуется test_datamatrix_codes.pdf для тестирования)
    ...
    ### Оптимизации:
    - ✅ Увеличено разрешение для лучшего распознавания
    - ✅ Оптимизация контрастности и резкости изображений
    - ✅ Подробное логирование времени выполнения
    - ✅ Быстрая обработка без генераторов
    """)
    ...
    **💡 Как выделить область:**
    ...
    6. Запустите быстрое сканирование
    
    **⚡ Оптимизации скорости:**
    - Увеличено разрешение рендеринга (3x вместо 2.5x)
    - Оптимизация контрастности и резкости
    - Убраны медленные генераторы
    - Подробное логирование времени выполнения
    """)


if __name__ == "__main__":
    print("⚡ Запуск GTIN Scanner Fast...")
    print("📱 Приложение откроется в браузере")
    print("📝 Логи записываются в файл gtin_scanner_fast.log")
    app.launch(
