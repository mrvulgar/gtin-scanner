#!/usr/bin/env python3
"""
Веб-приложение для извлечения GTIN data matrix кодов из PDF файлов с отладкой
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gtin_scanner.log'),
...
class GTINScanner:
    def __init__(self):
        ...
        logger.info("GTINScanner инициализирован")
    ...
    def load_pdf_preview(self, pdf_file):
        ...
        try:
            ...
            logger.info(f"Первая страница загружена: {page}")
    ...
    def scan_pdf_simple(self):
        """Простое сканирование PDF без генератора"""
        logger.info("scan_pdf_simple начато")
        ...
        if self.crop_rect is None:
            ...
            return "❌ Область не выбрана", None, "Выделите область с QR кодом кликом мыши"
        
        logger.info(f"Начинаем сканирование с областью: {self.crop_rect}")
        ...
        try:
            for page_num in range(total_pages):
                ...
                try:
                    ...
                    logger.info(f"На странице {page_num + 1} найдено {len(decoded_objects)} объектов")
            ...
            logger.info(f"Сканирование завершено. Найдено {len(all_codes)} кодов")
...
scanner = GTINScanner()
logger.info("GTIN Scanner приложение запущено")


# Создаем интерфейс Gradio
with gr.Blocks(title="GTIN Scanner Debug", theme=gr.themes.Soft()) as app:
    gr.Markdown("""
    # 🔍 GTIN Scanner Debug - Извлечение Data Matrix кодов из PDF
    
    ### Инструкция:
    1. **Загрузите PDF файл** в поле ниже (рекомендуется test_qr_codes.pdf для тестирования)
    ...
    ### Отладка:
    - Все действия логируются в файл `gtin_scanner.log`
    ...
    **🐛 Отладка:** Проверьте файл `gtin_scanner.log` для подробной информации о процессе сканирования.
    """)


if __name__ == "__main__":
    print("🚀 Запуск GTIN Scanner Debug...")
    print("📱 Приложение откроется в браузере")
    print("📝 Логи записываются в файл gtin_scanner.log")
    app.launch(
