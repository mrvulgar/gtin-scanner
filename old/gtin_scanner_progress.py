#!/usr/bin/env python3
"""
Веб-приложение для извлечения Data Matrix кодов из PDF файлов с прогресс-баром
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gtin_scanner_progress.log'),
...
class GTINScanner:
    def __init__(self):
        ...
        self.scanning = False
        self.progress_data = {"current": 0, "total": 0, "found": 0, "status": ""}
        
        logger.info("GTINScanner Progress инициализирован")
    ...
    def scan_pdf_with_progress(self, max_pages=None):
        ...
        if self.pdf_document is None:
            ...
            return "❌ PDF файл не загружен", None, "Загрузите PDF файл", gr.update()
        
        if self.crop_rect is None:
            logger.warning("Область обрезки не выбрана")
            return "❌ Область не выбрана", None, "Выделите область с Data Matrix кодом кликом мыши", gr.update()
        
        if self.scanning:
            return "⚠️ Сканирование уже выполняется", None, "Дождитесь завершения текущего сканирования", gr.update()
            ...
            try:
                ...
                for page_num in range(total_pages):
                    ...
                    self.progress_data.update({
                        ...
                        "total": total_pages,
                        "found": len(all_codes),
                        "status": f"Обработка страницы {page_num + 1}/{total_pages}"
                    })
    ...
    def get_progress(self):
        ...
        if self.scanning:
            progress_value = (self.progress_data["current"] / self.progress_data["total"]) * 100 if self.progress_data["total"] > 0 else 0
            stats = f"Страниц: {self.progress_data['current']}/{self.progress_data['total']} | Кодов: {self.progress_data['found']}"
            return self.progress_data["status"], stats
        else:
            return "Готов к работе", "Ожидание запуска сканирования"
...
scanner = GTINScanner()
logger.info("GTIN Scanner Progress приложение запущено")


# Создаем интерфейс Gradio
with gr.Blocks(title="GTIN Scanner Progress", theme=gr.themes.Soft()) as app:
    gr.Markdown("""
    # ⚡ GTIN Scanner Progress - Быстрое извлечение Data Matrix кодов из PDF
    ...
    # Автообновление прогресса каждые 2 секунды
    demo = gr.Interface(
        fn=scanner.get_progress,
        inputs=[],
        outputs=[scan_status, stats_display],
        live=True,
        update_every=2
    )
    ...
    **⚡ Оптимизации скорости:**
    ...
    - Фоновое сканирование с прогресс-баром
    - Возможность ограничить количество страниц для тестирования
    """)


if __name__ == "__main__":
    print("⚡ Запуск GTIN Scanner Progress...")
    print("📱 Приложение откроется в браузере")
    print("📝 Логи записываются в файл gtin_scanner_progress.log")
    app.launch(
