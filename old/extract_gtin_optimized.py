#!/usr/bin/env python3
"""
ОПТИМИЗИРОВАННАЯ версия: обработка только нужной области страницы (левый нижний угол)
Ускорение: ~5x за счёт обработки только 20% изображения
"""
...
def process_page_optimized(pdf_path, page_num, scale=3.0, crop_percent=0.25):
    """
    Обрабатывает ТОЛЬКО левый нижний угол страницы для максимальной скорости
    
    Args:
        pdf_path: путь к PDF файлу
        page_num: номер страницы
        scale: масштаб для рендеринга (3.0 = высокое качество)
        crop_percent: процент изображения для обработки (0.25 = 25%)
    ...
    try:
        ...
        # ОПТИМИЗАЦИЯ: Рендерим только нужную область (левый нижний угол)
        # Создаём clip rect для левого нижнего угла
        # Берём левую часть (25% ширины) и нижнюю часть (25% высоты от низа)
        clip_rect = fitz.Rect(
            0,                              # left
            rect.height * (1 - crop_percent),  # top (начинаем с 75% от верха)
            rect.width * crop_percent,      # right (25% ширины)
            rect.height                     # bottom (до конца)
        )
        
        # Рендерим только эту область с высоким качеством
...
def extract_datamatrix_from_pdf_optimized(pdf_path, output_csv_path, max_workers=12, scale=3.0, crop_percent=0.25):
    """
    БЫСТРАЯ извлечение data matrix кодов с прогресс-баром
    
    Args:
        pdf_path: путь к PDF файлу
        output_csv_path: путь к выходному CSV файлу
        max_workers: количество параллельных потоков
        scale: масштаб рендеринга (3.0 = высокое качество)
        crop_percent: процент области для обработки
    """
    print("\n" + "="*70)
    print(f"🚀 БЫСТРОЕ ИЗВЛЕЧЕНИЕ GTIN DATA MATRIX КОДОВ")
    print("="*70)
    ...
    print(f"⚡ Оптимизация: обработка {crop_percent*100:.0f}% изображения (левый нижний угол)")
    print(f"⏱  Ожидаемое время: ~{(total_pages * 0.6 / max_workers / 60):.1f} минут")
    print(f"\n⏳ Начинаем обработку...\n")
    ...
    with tqdm(total=total_pages, 
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Запускаем задачи
            futures = {
                executor.submit(process_page_optimized, pdf_path, page_num, scale, crop_percent): page_num
...
if __name__ == "__main__":
    ...
    NUM_WORKERS = 12   # Увеличено количество потоков
    SCALE = 3.0        # Высокое качество
    CROP_PERCENT = 0.25  # Обрабатываем только 25% изображения (левый нижний угол)
    
    codes = extract_datamatrix_from_pdf_optimized(
        pdf_file, 
        output_file, 
